...
int iniciarCliente(char* ip, int puerto) {  //para cliente de la cpu y de la memoria
...
void serializarPCB(int socket, PCB* pcb, t_tipoMensaje tipoMensaje) {
	...
	offset += sizeof(uint32_t);
	memcpy(stream + offset, &pcb->estimacion_anterior, sizeof(uint32_t));
	offset += sizeof(uint32_t);
	memcpy(stream + offset, &pcb->real_anterior, sizeof(uint64_t));
...
PCB* deserializoPCB(t_buffer* buffer){
	...
	stream += sizeof(uint32_t);
	memcpy(&(pcb->estimacion_anterior), stream, sizeof(uint32_t));
	stream += sizeof(uint32_t);
	memcpy(&(pcb->real_anterior), stream, sizeof(uint64_t));
