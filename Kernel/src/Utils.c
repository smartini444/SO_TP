...
t_configKernel extraerDatosConfig(t_config* archivoConfig) {
	...
	configKernel.puertoCPUDispatch = config_get_int_value(archivoConfig, "PUERTO_CPU_DISPATCH");
	configKernel.puertoCPUInterrupt = config_get_int_value(archivoConfig, "PUERTO_CPU_INTERRUPT");
...
void enviarInterruptCPU() {
	...
	string_append(&mensajeACpuInterrupt, "Hola esto es un mensaje de prueba");
	...
	log_debug(loggerKernel,"\n/////////////////////Envie el mensaje a la cpu interrupt\n");
...
void cerrarKernel() {
	...
	void _PCB_destroyer(void* elemento) {
		//list_destroy(((PCB*)elemento)->instrucciones);
		free(((PCB*)elemento));
	}
